		...
		if (fang->IsActive()) {
			...
			if (onimaruBullet) {
				...
				if (bullet) {
					...
					if (onimaruBulletcript) onimaruBulletcript->SetOnimaruDirection(GetSlightRandomSpread(0, maxSpread) * onimaruGunTransform->GetGlobalRotation());
				}
			}
		}
	}
...
Quat PlayerController::GetSlightRandomSpread(float minValue, float maxValue) const {

	float sign = rand() % 2 < 1 ? 1.0f : -1.0f;

	float4 axis = float4(onimaruGunTransform->GetUp(), 1);
